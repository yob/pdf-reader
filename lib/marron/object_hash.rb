...
require 'marron/buffer'
require 'marron/error'
require 'marron/object_cache'
...
module Marron
  ...
  # == Basic Usage
  #
  #     h = Marron::ObjectHash.new("somefile.pdf")
  #     h[1]
  #     => 3469
  #
  #     h[Marron::Reference.new(1,0)]
  #     => 3469
  #
  class ObjectHash
    include Enumerable
    ...
    def stream?(ref)
      self.has_key?(ref) && self[ref].is_a?(Marron::Stream)
    end

    # Access an object from the PDF. key can be an int or a Marron::Reference
    # object.
    #
    # If an int is used, the object with that ID and a generation number of 0 will
    # be returned.
    #
    # If a Marron::Reference object is used the exact ID and generation number
    # can be specified.
    #
    def [](key)
      return default if key.to_i <= 0

      unless key.is_a?(Marron::Reference)
        key = Marron::Reference.new(key.to_i, 0)
      end
      ...
      elsif xref[key].is_a?(Marron::Reference)
    ...
    # If key is a Marron::Reference object, lookup the corresponding
    # object in the PDF and return it. Otherwise return key untouched.
    #
    def object(key)
      key.is_a?(Marron::Reference) ? self[key] : key
    end
    alias :deref :object

    # Recursively dereferences the object refered to be +key+. If +key+ is not
    # a Marron::Reference, the key is returned unchanged.
    #
    def deref!(key)
    ...
    # can be an int or a Marron::Reference
    #
    def has_key?(check_key)
      # TODO update from O(n) to O(1)
      each_key do |key|
        if check_key.kind_of?(Marron::Reference)
    ...
    def to_s
      "<Marron::ObjectHash size: #{self.size}>"
    end
    ...
    # returns an array of Marron::References. Each reference in the
    ...
    def build_security_handler(opts = {})
      ...
      when :Standard
        ...
        raise Marron::EncryptedPDFError, "Unsupported encryption method (#{enc[:Filter]})"
      end
    end
    ...
    def new_buffer(offset = 0)
      Marron::Buffer.new(@io, :seek => offset)
    end
