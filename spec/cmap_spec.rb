...
describe "PDF::Reader::CMap with a bfchar cmap" do
  ...
  it "should correctly load a cmap object string" do
    ...
    map.map[0x1][0].should eql(0x48)
    map.map[0x2][0].should eql(0x65)
    map.map[0x9][0].should eql(0x73)
  end

  it "should correctly convert a character code into a unicode codepoint" do
    filename = File.dirname(__FILE__) + "/data/cmap_with_bfchar.txt"
    map = PDF::Reader::CMap.new(binread(filename))
    map.decode(0x1)[0].should eql(0x48)
    map.decode(0x2)[0].should eql(0x65)
    map.decode(0x9)[0].should eql(0x73)
  end
  ...
  it "should correctly load a cmap that uses the beginbfrange operator" do
    ...
    map = PDF::Reader::CMap.new(binread(filename))
    map.decode(0x16C9)[0].should eql(0x4F38) # mapped with the bfchar operator
    map.decode(0x0003)[0].should eql(0x0020) # mapped with the bfrange operator
    map.decode(0x0004)[0].should eql(0x0020+1) # mapped with the bfrange operator
    map.decode(0x0005)[0].should eql(0x0020+2) # mapped with the bfrange operator
  end

  it "should correctly load a cmap that uses the beginbfrange operator" do
    filename = File.dirname(__FILE__) + "/data/cmap_with_bfrange_two.txt"
    map = PDF::Reader::CMap.new(binread(filename))
    map.decode(0x0100)[0].should eql(0x0100) # mapped with the bfrange operator
  end
  ...
  it "should correctly load a cmap that uses the beginbfrange operator with the array syntax" do
    ...
    map.decode(0x00)[0].should eql(0xfffd) # mapped with the bfrange operator
    map.decode(0x01)[0].should eql(0x0050) # mapped with the bfrange operator
    map.decode(0x03)[0].should eql(0x0067) # mapped with the bfrange operator
    map.decode(0x08)[0].should eql(0x0073) # mapped with the bfrange operator
  end
