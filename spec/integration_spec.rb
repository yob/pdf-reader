...
# Add an open method to Marron:ObjectHash. This is to ease the migation for
# ObjectHash living in the pdf-reader gem, but eventually I'd like to update
# the specs to not need it.
module Marron
  class ObjectHash
    def self.open(input, opts = {}, &block)
      hash = self.new(input, opts)
      yield hash
    end
  end
end

describe Marron, "integration specs" do

  context "cairo-unicode-short" do
    let(:filename) { pdf_spec_file("cairo-unicode-short") }

    it "interprets unicode strings correctly" do
      Marron::ObjectHash.open(filename) do |reader|
        expect(reader.size).to eql(12)
      end
    end
  end
  ...
  context "PDF that has a content stream with a broken string" do
    ...
    it "doesn't hang when extracting doc info" do
      Timeout::timeout(3) do
        expect {
          reader = Marron::ObjectHash.new(filename)
          reader.size
        }.to raise_error(Marron::MalformedPDFError)
      end
    end
  end
  ...
  context "PDF with a stream that has its length specified as an indirect reference" do
    ...
    it "extracts text correctly" do
      Marron::ObjectHash.open(filename) do |reader|
        expect(reader.size).to eql(8)
      end
    end
  end

  # Marron::XRef#object was saving an incorrect position when seeking. We
  ...
  context "Empty file" do
    it "raises an exception" do
      expect {
        hash = Marron::ObjectHash.new(StringIO.new(""))
        hash.size
      }.to raise_error(Marron::MalformedPDFError)
    end
  end
  ...
  context "encrypted_version1_revision2_40bit_rc4_user_pass_apples" do
    ...
    context "with the user pass" do
      ...
      it "correctly extracts text" do
        Marron::ObjectHash.open(filename, :password => pass) do |reader|
